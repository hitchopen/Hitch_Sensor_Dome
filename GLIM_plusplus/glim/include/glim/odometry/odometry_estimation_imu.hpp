...
public:
   ...
   *
   * Forwards to NaiveInitialStateEstimation::set_init_state on the
   * internal init_estimation. The constructor always instantiates
   * NaiveInitialStateEstimation in this fork (LOOSE mode is no longer
   * selected from config), so the cast is always valid.
   */
  virtual void set_init_state(
    const Eigen::Isometry3d& T_world_imu,
    const Eigen::Vector3d& v_world_imu) override;
