...
// Ported from DLIO_plusplus art-jazzy (GLIM/glim/include/glim/util/urdf_transforms.hpp).
// Canonical location in the glim core package so both glim_ros2 (lidar_concat,
// glim_ros, glim_rosbag) and glim_ext (gnss_global lever-arm) can include it.
...
inline std::unordered_map<std::string, std::pair<std::string, Eigen::Isometry3d>> parse_urdf_transforms(const std::string& urdf_path) {
  ...
  // other thread can still be using libxml2. This function runs per-parse
  // (including from callback threads), where the global teardown could race
  // any other in-process libxml2 user.
  return transforms;
}
