...
OdometryEstimationIMU::OdometryEstimationIMU(std::unique_ptr<OdometryEstimationIMUParams>&& params_) : params(std::move(params_)) {
  ...
  // Hitch Sensor Dome fork: gravity-from-accelerometer initialization
  // is removed. We always construct NaiveInitialStateEstimation and
  // wait for the ROS wrapper to call set_init_state() with the Atlas
  // Duo INS pose; the LOOSE pathway and the implicit gravity-derivation
  // path are no longer reachable. See GLIM/docs/moving_start_initialization.md.
  //
  // Config keys (initialization_mode, estimate_init_state,
  // init_T_world_imu, init_v_world_imu) are kept on
  // OdometryEstimationIMUParams for ABI continuity but their effect is
  // limited: only the static init_T_world_imu / init_v_world_imu values
  // are honoured at construction (as a starting point), and they are
  // overwritten by the runtime INS /pose once it arrives.
  {
    auto naive_init = new NaiveInitialStateEstimation(params->T_lidar_imu, params->imu_bias);
    if (params->initialization_mode != "NAIVE") {
      logger->info(
        "Hitch fork: ignoring initialization_mode='{}' from config — "
        "always using NaiveInitialStateEstimation with external INS prior",
        params->initialization_mode);
    }
    if (!params->estimate_init_state) {
      // Honour any static init pose written into the JSON. The ROS
      // wrapper's runtime set_init_state will overwrite it once the
      // Atlas Duo /pose arrives.
      naive_init->set_init_state(params->init_T_world_imu, params->init_v_world_imu);
      logger->info("Hitch fork: applied static init_T_world_imu from config; "
                   "will be overwritten when INS /pose arrives.");
    }
    this->init_estimation.reset(naive_init);
  }
...
void OdometryEstimationIMU::set_init_state(
  const Eigen::Isometry3d& T_world_imu,
  const Eigen::Vector3d& v_world_imu) {
  // Hitch Sensor Dome fork: forward to NaiveInitialStateEstimation.
  // The constructor always instantiates Naive (LOOSE was removed in
  // this fork), so the dynamic_cast is always valid for live sessions.
  // Once the optimizer has produced its first frame, init_estimation is
  // released (frames container takes over) — late calls become no-ops.
  if (!init_estimation) {
    logger->warn("set_init_state called after init_estimation was released — "
                 "ignoring (the optimizer is already past initialization).");
    return;
  }
  auto* naive = dynamic_cast<NaiveInitialStateEstimation*>(init_estimation.get());
  if (!naive) {
    logger->error("set_init_state: init_estimation is not Naive — this should "
                  "not happen in the Hitch fork. Ignoring.");
    return;
  }
  naive->set_init_state(T_world_imu, v_world_imu);

  // Also keep the params struct in sync so anything that reads it (e.g.
  // serialization, debugging) sees the actually-applied values.
  params->init_T_world_imu = T_world_imu;
  params->init_v_world_imu = v_world_imu;
  params->estimate_init_state = false;
  logger->info("Hitch fork: external INS init pose accepted.");
}
