...
void AsyncOdometryEstimation::set_init_state(
  const Eigen::Isometry3d& T_world_imu,
  const Eigen::Vector3d& v_world_imu) {
  // Hitch Sensor Dome fork: stash the external init state. The worker
  // thread (run()) drains it before its next iteration so the
  // estimator's set_init_state is invoked from a single thread without
  // racing insert_imu / insert_frame. Idempotent — only the first call
  // has effect; subsequent calls are dropped.
  std::lock_guard<std::mutex> lock(pending_init_mutex);
  if (init_state_applied) {
    return;
  }
  pending_init_state = std::make_pair(T_world_imu, v_world_imu);
}
...
void AsyncOdometryEstimation::run() {
  ...
  while (!kill_switch) {
    // Hitch Sensor Dome fork: drain any pending external init state
    // BEFORE consuming inputs, so the estimator sees the INS pose
    // before the first IMU sample is forwarded.
    {
      std::lock_guard<std::mutex> lock(pending_init_mutex);
      if (pending_init_state.has_value() && !init_state_applied) {
        odometry_estimation->set_init_state(
          pending_init_state->first, pending_init_state->second);
        init_state_applied = true;
        pending_init_state.reset();
      }
    }
