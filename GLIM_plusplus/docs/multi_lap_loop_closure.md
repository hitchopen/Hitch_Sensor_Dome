...
## Important context — the GNSS factor stream

The GNSS-factor parameters discussed below (`prior_inf_scale = [1e4, 1e4, 5e4]`, `min_baseline = 0.5`) only take effect when libgnss_global.so is actually receiving messages. Earlier revisions of this fork pointed the module directly at `/gps/fix` (`gnss_msg_type: "sensor_msgs/msg/NavSatFix"`) — but the upstream module's dispatcher only handles `nav_msgs/msg/Odometry` and `geometry_msgs/msg/PoseWithCovarianceStamped`, so the subscription silently dropped every NavSatFix and the tuning was a no-op.

The current fix routes GNSS through an RTK-gated bridge in glim_ros2 (`GlimROS::try_publish_gnss_factor`). The bridge subscribes to `/pose` + `/gps/fix`, applies the same RTK-fixed gate the init pose uses, and republishes accepted poses on `/gnss/pose_rtk_only` as `PoseWithCovarianceStamped`. libgnss_global.so subscribes there. During RTK-float / no-fix periods the bridge silently suspends — gnss_global's queue stays empty and no factors are added, which is the correct behavior (false GNSS factors would corrupt the global anchor). When RTK locks again, factors resume on the next `/pose`. See [`moving_start_initialization.md`](moving_start_initialization.md) for the full bridge design.
...
```bash
ros2 launch GLIM_plusplus/launch/hitch_sensor_dome.launch.py 2>&1 | tee glim_run.log
```
